...
**이 변경을 적용했을 때** 의도치 않게 발생할 수 있는 부작용을 분석합니다.

- **영향 범위**: 변경으로 인해 동작이 달라질 수 있는 모듈/파일 목록
- **예상 부작용**: 이 수정이 유발할 수 있는 문제 (기존 동작 변경, 성능 저하, 데이터 포맷 변경 등)
- **하위 호환성**: 기존 API/인터페이스 계약이 깨지는지 여부
- **전파 경로**: 변경이 어떤 경로로 다른 모듈에 영향을 미치는지 (import 체인, 이벤트, 공유 상태 등)
...
> 이 변경을 적용했을 때 발생할 수 있는 부작용

- **영향 범위**: {변경으로 동작이 달라질 수 있는 모듈/파일}
- **전파 경로**: {변경이 전파되는 경로 — import 체인, 이벤트, 공유 상태 등}
- {예상 부작용 1: 구체적 시나리오 + 영향}
- {예상 부작용 2: 구체적 시나리오 + 영향}

## 총정리

| 항목 | 판단 | 변경 필요도 | 핵심 요약 |
|------|------|-------------|-----------|
| ROI | {높음/보통/낮음} | {매우 낮음~매우 높음} | {한 줄 요약} |
| 제약사항 | {N}건 | {매우 낮음~매우 높음} | {가장 중요한 제약} |
| 엣지케이스 | {N}건 ({미처리}건 미처리) | {매우 낮음~매우 높음} | {가장 위험한 케이스} |
| 사이드이펙트 | {N}건 | {매우 낮음~매우 높음} | {가장 큰 영향} |

**변경 필요성**: {매우 낮음/낮음/보통/높음/매우 높음} — {4개 항목의 변경 필요도를 종합한 1-2문장 근거}

**종합 의견**: {변경사항에 대한 1-2문장 종합 판단 — 진행 권장/주의 필요/재검토 권장}

**한 줄 요약**: [ROI: {판단}] [제약: {N}건] [엣지케이스: {N}건] [사이드이펙트: {N}건] [변경 필요성: {단계}]
