...
│   │   │   ├── api/              # REST 컨트롤러
│   │   │   ├── service/          # 비즈니스 로직
│   │   │   ├── config/           # 설정 (Security, JPA, FCM, WebSocket 등)
...
- 웹과 앱이 동일한 API를 사용 — 플랫폼별 분기는 최소화
...
<!-- 예시:
- 웹과 앱 모두 동일한 REST API를 사용
- 인증 토큰은 동일한 JWT 포맷
- 푸시 알림은 앱 전용 (FCM) — 웹은 WebSocket/SSE로 실시간 알림
- 디바이스 토큰 등록 API는 앱에서만 호출: `POST /api/v1/devices`
-->
